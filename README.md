...
To solve this problem I will employ machine learning. Based on the provided log data, I will train a classification 
algorithm to predict from all other log messages, whether a user's log contains an account cancellation event. This 
should lead to a classification model, which will already classify a user as churning, some time before they actually 
cancel. To obtain the best possible model, I will train and evaluate three classification algorithms: logistic 
regression, random forest classification and linear support vector classification. Each of these algorithms will be 
trained on per-user features aggregated from a small exploration data set. This set is generated by selecting all log 
entries of a small selection of users. It is vital not to sample stratified by users, otherwise the aggregated features 
would be rendered useless. By training on a grid of hyperparameters and evaluating on a test dataset, the best 
performing algorithm / hyperparameter combination will be chosen. This will be employed to train a production model 
on a much larger data set.

I will supply a web application which shows a dashboard displaying statistics on the trained model and the feature 
distribution in the training data set. It will allow uploading log data and will output the churn prediction for each 
user ID present in the uploaded data.
